...
Procedure.f RandomWeight()
  ProcedureReturn (Random(200) - 100) / 1000.0  ; Smaller initial weights
EndProcedure
...
Procedure Train(*nn.NeuralNetwork, epochs.i, learningRate.f)
  ...
  For i = 0 To epochs - 1
    For j = 0 To ArraySize(odata())
      inputs(0) = odata(j)\a / 100.0 ; Normalizing inputs
      inputs(1) = odata(j)\b / 100.0 ; Normalizing inputs
      
      If odata(j)\operator = "+"
        targetOutput = odata(j)\result / 200.0 ; Normalizing output
...
Procedure.f TestNetwork(*nn.NeuralNetwork, a.i, b.i, operator.s)
  ...
  inputs(0) = a / 100.0 ; Normalizing inputs
  inputs(1) = b / 100.0 ; Normalizing inputs
  
  ForwardPass(*nn, @inputs(), @hiddenOutputs(), @finalOutput)
  
  ProcedureReturn finalOutput * 200.0 ; Denormalizing output
EndProcedure

; Train the neural network
Train(@nn, 10000, 0.01) ; Reduced learning rate
...
Debug "Test result: " + StrF(result, 2)
